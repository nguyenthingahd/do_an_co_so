...
exports.list = () => {
  return new Promise((hamOK, hamLoi) => {
    let sql = "SELECT * FROM laptop_shop.product";
    db.query(sql, (err, d) => {
...
exports.detail = (idProduct) => {
  return new Promise((hamOK, hamLoi) => {
    let sql = `SELECT * FROM laptop_shop.product WHERE idProduct=${idProduct}`;
    db.query(sql, (err, d) => {
      console.log("List success");
      data = d[0];
      hamOK(data);
...
exports.delete = (idProduct) => {
  let sql = `DELETE FROM laptop_shop.product WHERE idProduct=${idProduct}`;
  let query = db.query(sql, (err, result) => {
...
exports.hotProduct = () => {
  return new Promise((hamOK, hamLoi) => {
    let sql =
      "SELECT * FROM laptop_shop.product WHERE views > 0 ORDER BY views DESC LIMIT 5";
    db.query(sql, (err, d) => {
      console.log("List success");
      dataList = d;
      hamOK(dataList);
...
exports.newProduct = () => {
  return new Promise((hamOK, hamLoi) => {
    let sql =
      "SELECT * FROM laptop_shop.product ORDER BY dateUpdate DESC LIMIT 5";
    db.query(sql, (err, d) => {
      console.log("List success");
      dataList = d;
      hamOK(dataList);
